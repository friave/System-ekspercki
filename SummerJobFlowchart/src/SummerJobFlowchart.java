...
class SummerJobFlowchart implements ActionListener{
	
	   JLabel displayLabel;
	   JButton nextButton;
	   JPanel choicesPanel;
	   ButtonGroup choicesButtons;
	   ResourceBundle jobsResources;
	   ...
	   SummerJobFlowchart(){
		      try
		        {
		         jobsResources = ResourceBundle.getBundle("resources.SummerResources",Locale.getDefault());
		        }
		      ...
		      JFrame jfrm = new JFrame(jobsResources.getString("SummerDemo"));  
		      ...
		      nextButton = new JButton(jobsResources.getString("Next"));
	   ...
	   private void nextUIState() throws Exception
	     {
	      ...
	      if (fv.getFactSlot("state").toString().equals("final"))
	        { 
	         ...
	         nextButton.setText(jobsResources.getString("Restart")); 
	        }
	      else if (fv.getFactSlot("state").toString().equals("initial"))
	        {
	         nextButton.setActionCommand("Next");
	         nextButton.setText(jobsResources.getString("Next"));
	        }
	      else
	        { 
	         nextButton.setActionCommand("Next");
	      ...
	      for (int i = 0, j = 0; (i < pv1.size()) && (j < pv2.size()); i++, j++)
	        {
	         ...
	         if (bv1.toString().equals(selected))
	            { rButton = new JRadioButton(jobsResources.getString(bv2.getValue().toString()),true); }
	         else
	            { rButton = new JRadioButton(jobsResources.getString(bv2.getValue().toString()),false); }
	      ...
	      String theText = jobsResources.getString(fv.getFactSlot("display").symbolValue());
	   ...
	   private void wrapLabelText(
		...
		public static void main(String[] args)      {  
		      ...
		      SwingUtilities.invokeLater(
		        new Runnable() 
		          {  
		           public void run() { new SummerJobFlowchart(); }  
		          });   
		     }  
